...
import { login, saveCurrentUser } from '../utils/auth';
...
const LoginModal = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const result = login(formData.email, formData.password, userType);
    ...
    if (result.success) {
      ...
      // Redirection automatique pour TOUS les rôles (sans alert)
      setTimeout(() => {
        if (window.onAdminLogin) {
          window.onAdminLogin(result.user);
        }
      }, 100);
